...
const addCustomRecordDialogIsOpened = defineModel('addCustomRecordDialogIsOpened', { default: false, type: Boolean })

customRecordStore.fetchAll()

const updateAddCustomRecordDialogIsOpened = (value: boolean) => {
  addCustomRecordDialogIsOpened.value = value
}
</script>
...
<template>
  <v-navigation-drawer v-model="drawerIsOpened">
    ...
    <v-divider />
    <div class="pa-4">
      <v-btn variant="outlined" class="w-100" @click="addCustomRecordDialogIsOpened = true">
    ...
    <v-list color="transparent">
      <v-list-item
        ...
        :key="tracking.name"
        :to="`suivi-${slugify(tracking.name)}`"
        variant="flat"
        :title="tracking.name"
        @click="drawerIsOpened = !drawerIsOpened" />
    </v-list>
    ...
    <AddCustomRecord
      v-model="addCustomRecordDialogIsOpened"
      @update:add-custom-record-dialog-is-opened="updateAddCustomRecordDialogIsOpened" />
  </v-navigation-drawer>
</template>
